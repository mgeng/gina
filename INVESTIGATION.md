...
> **注記 (リファクタ後)**: 本レポートは調査当時のスナップショット。以下の行番号・「main.js に集約」等の記述は **当時のもの**で現状とは異なる。
> 現在はレンダリング中核（テキスト/フキダシ描画・計測・フォント一覧・関連定数）を **`src/core/renderer.js` に一本化**し、GUI (`src/main.js`) と CLI (`bin/gina-render.js`) の双方が同一実装を共有する（重複コピーなし）。
